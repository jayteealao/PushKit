...
- **PyPI:** `pushkit==0.1.0rc1` published via OIDC Trusted Publishing (PEP 440 normalised).
...
Default components install `pushkit-server.exe` to `%ProgramFiles%\PushKit\`, add a Start Menu shortcut, and register an Apps & Features entry. The Windows service component (default-checked) registers `PushKitServer` as a manual-start service via `sc.exe`.
